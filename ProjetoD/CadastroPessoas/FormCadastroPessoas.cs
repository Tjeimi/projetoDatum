...
namespace CadastroPessoas {
    public partial class FormCadastroPessoas : Form {
        ...
        public FormCadastroPessoas(long? idPessoaExt) {
            InitializeComponent();
            tbIdPessoa.Text = Convert.ToString(idPessoaExt);
        }

        #region EVENTOS
        private async void FormCadastroPessoas_Load(object sender, EventArgs e) {
            if (tbIdPessoa.Text != "") {
                // select por id
        ...
        private void RetornarSelect(string r) {
            MethodInvoker methodInvokerDelegate = delegate () {
                ...
                var resposta = JsonSerializer.Deserialize<BasePacketResposta>(r);
                if (resposta!.codigo == 200) {
                    var pessoa = JsonSerializer.Deserialize<PessoasModel>(resposta.dados!)!;
                    //só para dar um feedback se deu certo
                    tbResultado.BackColor = Color.DarkGreen;
                    tbResultado.Text = resposta.mensagem;
                    ...
                    cbCidade.Text = pessoa.cidade;
                    cbEstado.Text = pessoa.estado;
                    tbContatoPessoa.Text = pessoa.fone;
                    tbEnderecoPessoa.Text = pessoa.endereco;
                    chbAtivo.Checked = pessoa.ativo ?? false;
                } else {
                    //só para dar um feedback se deu certo
                    tbResultado.BackColor = Color.DarkRed;
                    tbResultado.Text = resposta.mensagem;
